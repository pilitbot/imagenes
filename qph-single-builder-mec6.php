...
final class QPH_Single_Builder_MEC {
    ...
    // SETTINGS - SOLO JavaScript (NO hook PHP)
...
public function get_builder_style_label($label, $style) {
    ...
    (function($) {
        ...
        function qphInject() {
            ...
            // Ya inyectado?
            if ($('#mec_settings_single_style_builder').length > 0) {
                console.log('[QPH-ESB] Ya existe');
                qphBind();
                return true;
            }

            // ================================================
            // Buscar el contenedor EXACTO de MEC
            // ID confirmado: #mec_settings_single_event_style
            // ================================================
            var $container = $('#mec_settings_single_event_style');

            if ($container.length === 0) {
                console.log('[QPH-ESB] Contenedor no encontrado aún');
                return false;
            }

            console.log('[QPH-ESB] ✅ Contenedor #mec_settings_single_event_style encontrado');

            var show = isBuilder ? '' : 'display:none;';
            var bOpts = buildOpts(builders, selBuild);
            var mOpts = buildOpts(builders, selModal);
            var eOpts = buildOpts(events, selEvent);

            var noTpl = builders.length === 0
                ? 'Please Create New Design '
                  + '<a href="' + createUrl + '" class="taxonomy-add-new">Create new</a>'
                : '';

            var sSel = builders.length > 0
                ? '<label class="mec-col-3">Default Builder for Single Event</label>'
                  + '<div class="mec-col-9">'
            ...
            var mSel = builders.length > 0
                ? '<label class="mec-col-3">Default Builder for Modal View</label>'
                  + '<div class="mec-col-9">'
            ...
            // ================================================
            // Construir HTML
            // Radio DENTRO de #mec_settings_single_event_style
            // ================================================

            // Radio button (va DENTRO del contenedor)
            var radioHtml = '<div class="mec-form-row">'
            ...
            // Opciones adicionales (van DESPUÉS del contenedor)
            var optsHtml = ''
                ...
                + ' style="' + show + '">'
                + '<label class="mec-col-3">Custom Event For Set Settings</label>'
                + '<div class="mec-col-9">'
            ...
            // ================================================
            // INSERTAR:
            // 1. Radio DENTRO de #mec_settings_single_event_style
            //    (append = al final del contenedor)
            // 2. Opciones DESPUÉS de #mec_settings_single_event_style
            // ================================================
            $container.append(radioHtml);
            $container.after(optsHtml);

            console.log('[QPH-ESB] ✅ Radio insertado DENTRO del contenedor');
            console.log('[QPH-ESB] ✅ Opciones insertadas DESPUÉS del contenedor');
        ...
        function qphBind() {
            ...
            $(document).on(
                ...
                function() {
                    ...
                    v === 'builder' ? $w.slideDown(300) : $w.slideUp(300);
                }
            );
        }
        ...
        function tryIt() {
            att++;
            console.log('[QPH-ESB] Intento ' + att);
            if (qphInject()) {
                console.log('[QPH-ESB] ✅ Completado en intento ' + att);
                return;
            }
            if (att < 30) setTimeout(tryIt, 300);
            else console.log('[QPH-ESB] ❌ Falló después de 30 intentos');
        }

        $(document).ready(function() {
            console.log('[QPH-ESB] v4.1 iniciado');
            tryIt();

            // Observer para tabs dinámicos
            var obs = new MutationObserver(function() {
                if ($('#mec_settings_single_event_style').length > 0
                    && $('#mec_settings_single_style_builder').length === 0) {
                    att = 0;
                    tryIt();
                }
            });
            ...
            // Click en tabs de MEC
            $(document).on('click', '.mec-settings-menu a, [data-id], .wn-mec-select', function() {
                setTimeout(function() {
                    if ($('#mec_settings_single_style_builder').length === 0) {
                        att = 0;
                        tryIt();
